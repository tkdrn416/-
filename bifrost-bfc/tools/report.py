...
"""data/history.csv → bifrost-daily-report.html
 - KPI + 전일/전주 Δ표 + 자동 내러티브
 - 정적 SVG 시계열(가격 USD/KRW, BIFI USD/KRW, 예치 vs 대출 달러단위 등)
 - ★ 커스텀 차트 빌더: 원하는 지표 골라 한 그래프에 겹쳐 비교(달러환산 담보 포함), 지수화 토글
...
WST_RATE=1.137  # wstBFC≈1.137 BFC
...
def augment(rows):
    ...
    for r in rows:
        ...
        bfc=_num(r.get("bifi_bfc_pool")); wst=_num(r.get("bifi_wstbfc")); btc=_num(r.get("bifi_btcusd"))
        ...
        r["coll_btcusd_usd"]=round(btc) if btc is not None else ""   # BtcUSD ≈ $1
        parts=[_num(r.get(k)) for k in ("coll_bfc_usd","coll_wstbfc_usd","coll_btcusd_usd")]
        r["coll_total_usd"]=round(sum(x for x in parts if x is not None)) if any(x is not None for x in parts) else ""
    return rows

# 빌더/차트에서 쓸 지표 카탈로그: (csv키, 라벨, 단위)
CATALOG=[
 ("price_usd","BFC 가격($)","$"),("price_krw","BFC 가격(원)","원"),
 ("bifi_price_usd","BIFI 가격($)","$"),("bifi_price_krw","BIFI 가격(원)","원"),
 ("coll_total_usd","BiFi 예치합계($)","$"),("bifi_borrow_dollar","BiFi 달러대출($)","$"),
 ...
 ("exch_total","거래소 보유(BFC)","BFC"),("btcusd_supply","BtcUSD 발행","BFC"),("jpyc_supply","일본 JPYC","BFC"),
 ("stbfc_supply","유동스테이킹(stBFC)","BFC"),("val_total_stake","검증자 스테이크","BFC"),("treasury_bfc","Treasury","BFC"),
...
def svg_chart(rows,cols,title,unit="",colors=None,h=190):
    ...
    drawn=[(lab,(pts*2 if len(pts)==1 else pts)) for lab,pts in series]
    ...
    g.append(f'<text x="{W-PADR}" y="18" fill="#9aa3b2" font-size="10" text-anchor="end">최신 {_fmt(series[0][1][-1][1],unit)}</text></svg>')
...
def change_table(rows):
    ...
    M=[("price_usd","BFC 가격(USD)","$"),("price_krw","BFC 가격(KRW)","원"),("bifi_price_usd","BIFI 가격(USD)","$"),("bifi_price_krw","BIFI 가격(KRW)","원"),
     ("exch_total","거래소 보유",""),("exch_net_flow","거래소 순흐름",""),("treasury_bfc","Treasury",""),("btcusd_supply","BtcUSD 발행",""),
     ("coll_total_usd","BiFi 예치($)","$"),("bifi_borrow_dollar","BiFi 달러대출($)","$"),("jpyc_supply","JPYC",""),("stbfc_supply","유동스테이킹",""),
     ("val_total_stake","검증자 스테이크",""),("nakamoto33","Nakamoto",""),("defi_tvl_usd","TVL","$")]
...
def narrative(rows):
    if len(rows)<2: return "데이터 1포인트 — 내러티브는 2회차부터."
    ...
    for col,lab,thr in [("exch_total","거래소",1_000_000),("treasury_bfc","Treasury",1),("btcusd_supply","BtcUSD",200_000),
      ("bifi_borrow_dollar","달러대출",100_000),("coll_total_usd","예치($)",500_000),("jpyc_supply","JPYC",200_000),("val_total_stake","검증자스테이크",1_000_000)]:
        x=d(col)
...
def generate():
    ...
    static=[
        ...
        svg_chart(rows,[("coll_total_usd","예치($)"),("bifi_borrow_dollar","대출($)")],"★ BiFi 예치 vs 대출 (달러 단위 나란히 비교)","$"),
        svg_chart(rows,[("coll_bfc_usd","BFC담보"),("coll_wstbfc_usd","wstBFC담보"),("coll_btcusd_usd","BtcUSD담보")],"BiFi 예치(담보) 달러 환산","$"),
        ...
        svg_chart(rows,[("treasury_bfc","Treasury")],"재단 Treasury(미집행)"),
        svg_chart(rows,[("alert_p1","P1"),("alert_p2","P2")],"일별 경보 건수"),
    ]
    # 빌더용 데이터
    dates=[r["date"] for r in rows]
    series={key:[_num(r.get(key)) for r in rows] for key,_,_ in CATALOG}
    cat=[{"k":k,"lab":lab,"u":u} for k,lab,u in CATALOG]
    DATA=json.dumps({"dates":dates,"series":series,"cat":cat})
    al=recent_alerts()
...
.lead{color:#9aa3b2;font-size:12.5px} .grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:10px;margin:10px 0}
...
.b-bad{color:#f87171;font-weight:700} .b-warn{color:#fbbf24;font-weight:700} .up{color:#f87171} .dn{color:#34d399} .flat{color:#6b7280}
.builder{background:#171a21;border:1px solid #34d399;border-radius:10px;padding:12px}
...
<div class="grid">
 <div class="kpi"><div class="v">${L('price_usd',lambda v:f'{v:.5f}')}</div><div class="l">BFC ({L('price_krw',lambda v:f'{v:,.1f}')}원)</div></div>
 <div class="kpi"><div class="v">${L('bifi_price_usd',lambda v:f'{v:.5f}')}</div><div class="l">BIFI 곡괭이 ({L('bifi_price_krw',lambda v:f'{v:,.2f}')}원)</div></div>
 <div class="kpi"><div class="v">{L('exch_total',lambda v:f'{v/1e6:,.1f}M')}</div><div class="l">거래소 보유 BFC</div></div>
 <div class="kpi"><div class="v">{L('btcusd_supply',lambda v:f'{v/1e6:,.2f}M')}</div><div class="l">BtcUSD 발행(홀더 {L('btcusd_holders')})</div></div>
 <div class="kpi"><div class="v">${L('coll_total_usd',lambda v:f'{v/1e6:,.2f}M')}</div><div class="l">BiFi 예치(달러환산)</div></div>
 <div class="kpi"><div class="v">${L('bifi_borrow_dollar',lambda v:f'{v/1e6:,.2f}M')}</div><div class="l">BiFi 달러코인 대출</div></div>
 <div class="kpi"><div class="v">{L('val_count')}·N{L('nakamoto33')}</div><div class="l">검증자·Nakamoto</div></div>
 <div class="kpi"><div class="v">${L('defi_tvl_usd',lambda v:f'{v/1e6:,.1f}M')}</div><div class="l">DefiLlama TVL</div></div>
</div>
<h2>🛠 커스텀 차트 빌더 — 원하는 지표를 골라 한 그래프에 겹쳐 비교</h2>
<div class="builder">
 <div id="picks"></div>
 <div><button class="btn" onclick="draw()">그리기</button>
  <button class="btn" onclick="preset('cmp')">예치vs대출($)</button>
  <button class="btn" onclick="preset('price')">BFC/BIFI 가격</button>
  <label style="border-left:1px solid #333;padding-left:10px"><input type="checkbox" id="idx"> 지수화(첫값=100, 단위 다른 지표 비교)</label></div>
 <div id="bchart" style="margin-top:8px"></div>
</div>
<h2>📊 전일/전주 대비 변화</h2>{change_table(rows)}
<h2>📈 시계열 그래프</h2><div class="charts">{''.join(static)}</div>
...
<p class="lead">전체 시계열=<code>data/history.csv</code>(엑셀) · 경보원본=<code>data/monitor-events.log</code> · ▲증가 ▼감소.</p>"""
    script="""<script>
var D=__DATA__;
var CL=["#4da3ff","#fbbf24","#34d399","#f87171","#b794ff","#f0883e","#56d4dd","#e879f9"];
...
function preset(p){var on=p==='cmp'?['coll_total_usd','bifi_borrow_dollar']:['price_usd','bifi_price_usd'];
 document.querySelectorAll('input[name=s]').forEach(function(e){e.checked=on.indexOf(e.value)>=0;});if(p==='price')document.getElementById('idx').checked=true;else document.getElementById('idx').checked=false;draw();}
function draw(){
 var keys=[];document.querySelectorAll('input[name=s]:checked').forEach(function(e){keys.push(e.value);});
 var idx=document.getElementById('idx').checked;var W=1080,H=360,PL=70,PR=140,PT=24,PB=34;
 var ser=keys.map(function(k){var lab=(D.cat.find(function(c){return c.k===k})||{}).lab||k;
   var pts=[];D.series[k].forEach(function(v,i){if(v!==null&&v!==undefined)pts.push([i,v]);});return {k:k,lab:lab,pts:pts};}).filter(function(s){return s.pts.length;});
 ...
 var lo=Math.min.apply(null,all),hi=Math.max.apply(null,all);if(hi===lo){hi=lo*1.001+1;lo=lo*0.999-1;}
 ...
 ser.forEach(function(s,si){var c=CL[si%CL.length];var p=norm(s);var pts=p.length===1?[p[0],p[0]]:p;
   var d=pts.map(function(pt,i){return (i?'L':'M')+' '+X(pt[0]).toFixed(0)+' '+Y(pt[1]).toFixed(0);}).join(' ');
   ...
   var last=s.pts[s.pts.length-1];g+='<text x="'+(W-PR+6)+'" y="'+(28+si*18)+'" fill="'+c+'" font-size="11">■ '+s.lab+'</text><text x="'+(W-PR+6)+'" y="'+(28+si*18+11)+'" fill="#9aa3b2" font-size="9">'+(idx?(last[1]/(s.pts[0][1]||1)*100).toFixed(0):fmt(last[1]))+'</text>';});
 g+='</svg>';if(idx)g+='<p class="muted small" style="margin:4px 0 0">지수화 모드: 각 지표 첫값=100 기준 상대변화(단위 다른 지표 형태 비교용).</p>';
 document.getElementById('bchart').innerHTML=g;}
function tgl(){}
</script></body></html>"""
