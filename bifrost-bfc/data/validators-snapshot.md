...
>
> **★ 위임 정체 규명(2026-06-30): 위임 344.76M의 ~78%가 Biquid 유동스테이킹 경유.** Biquid LST 풀 `totalPooledBFC()`=**268.8M BFC**를 검증자에 내부위임(공식문서: "가장 수익성높고 신뢰받는 검증자에 위임"). → "D형 retail 위임"의 상당분은 개별 위임이 아니라 **stBFC 홀더(130명)의 LST 물량**이 재위임된 것. 실질 위임 의사결정 주체는 130명 수준으로 집약. (감시: stBFC totalPooledBFC 급변=위임 이동, 특정 검증자로의 LST 재배분.)
