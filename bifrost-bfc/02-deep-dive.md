...
4. **StableDAO (2025-07-18 출범):** DAO 수익 30%로 BFC 소각 **계획 발표** (실제 집행은 미확인). ⚠️ 앞서 인용한 'Q3 ~50M/$1.45M 소각'은 **CMC AI 집계 페이지 출처로 신뢰 낮음·온체인 미확인 → 철회**. 온체인 dead 주소 소각 이력은 **2021-03-14 31.4M(Pi Day) + 2022-02-10 ~16억(재단 40%)뿐**이며 2025년 추가 소각 흔적 없음
...
- **예치 venue** `0x6894Ae31Cae97F228590F6dC7BBea7449F4DB980`(미검증): JPYC **99.99%(65.5M) 보유**, 2026-02-19 배포, 2026-06 현재도 활발(LIVE). **HashPort JPYC 4% 렌딩의 Bifrost측 예치 풀로 추정.**
- **예치 구조(실측·정정):** venue로 JPYC 보낸 주소 = **5개뿐, 전부 컨트랙트**(개별 retail EOA 아님). 메인 라우터 `0xcC56EABf`도 컨트랙트 2개(`0xb531a378`+venue 자신)에서 받아 인프라끼리 순환. → Bifrost 상에선 **소수 렌딩 인프라 컨트랙트가 JPYC 집계·보유**. 실제 다수 일반 예치자는 HashPort 레이어(브릿지/커스터디)에 있어 **Bifrost 온체인만으론 개별 식별 불가**.
- **⚠️ 자기정정:** 초안의 "JPYC=운용자 네트워크가 운영"은 **과대해석**(근거가 'venue 생성자=가스허브 자금수령' 하나뿐, 가스허브는 51지갑 범용 가스주입이라 약함; JPYC 보유 컨트랙트는 운용망 주소와 불일치). **HashPort 렌딩 인프라**로 보는 게 타당(사용자 예치 이벤트 결과 개연성 높음).
- **BtcUSD 당일 대형 상환:** 총공급 **6.11M**. 점진 수축 아니라 **오늘(2026-06-30) 하루 -1.96M 단일 소각**(전일 ~8.06M). 소각 = EOA 6곳 상환(0x85b44cf6 -1.19M·0xcca25978 -408K 등). 분포 극집중(상위 3곳 96%: 브릿지볼트 61%). 감시: `tools/monitor.py` JPYC/BtcUSD 공급추세.
